...
:description: Understanding how LLMs learn through prediction and correction at scale
...
. Compare its prediction to the actual next token
. Adjust the weights to make the prediction better
...
*You might be wondering:* "_How does backpropagation compute gradients? What does 'working backward' mean?_"
...
. *Throw* (forward pass): Predict where the dart lands
. *See the result* (loss): How far from bullseye?
. *Adjust your form* (backpropagation): Figure out what to change
. *Try again* (weight update): Incorporate the adjustment
. *Repeat*: Thousands of throws
...
|Mostly English
|English well, other languages poorly
...
|Parameters (weights)
|7 billion to 1 trillion+

|Training tokens
|1 trillion to 15+ trillion

|Training time
|Weeks to months

|GPUs
|Thousands of A100s or H100s
...
|4.0
|55
|Model is ~55-way confused at each token

|3.0
|20
|Model is ~20-way confused

|2.0
|7.4
|Model is ~7-way confused

|1.5
|4.5
|Model is ~4-way confused
|===
...
*You might be wondering:* "_If training costs millions, how do smaller companies afford to use LLMs?_"
...
|*Metadata storage*
|Store hyperparameters, metrics, training data references

|*Lineage tracking*
|Know which dataset and code produced each model

|*Deployment integration*
|Deploy directly from registry to model serving
|===
...
. *Inference*: Prompt → Tokenize → Embed → Transform → Predict → Sample → Repeat
. *Training*: Same forward pass + loss + backpropagation + weight update
...
xref:11-kv-cache.adoc[Next: The KV Cache — Why Inference Doesn't Crawl →]
