...
:description: Step-by-step walkthrough of how a prompt becomes a response

_Part V: Watching It Think_
...
. *Self-Attention*: Tokens gather information from relevant other tokens
. *Feed-Forward Network*: Each token is processed independently
. *Layer Normalization*: Values are kept in reasonable ranges
...
*You might be wondering:* "_Why use only the final token's vector to predict the next token? Why not use all tokens or an average?_"
...
*You might be wondering:* "_How can the same matrix work in both directions (lookup vs matrix multiply)?_"
...
*You might be wondering:* "_Which sampling strategy should I use in production?_"
...
|Excellent — highly predictable, high acceptance rate

|*Low temperature (0.1-0.5)*
|Good — draft model predictions are often accepted

|*High temperature (>1.0)*
|Poor — randomness leads to low acceptance rate, wasted computation

|*Beam search*
|Incompatible — fundamentally different decoding approach
|===
...
|Embedding lookup
|GPU
|Accessing model weights in GPU memory
...
|Sampling
|CPU (often)
|Probability calculations and random selection
...
. The *CPU* receives your text and runs the tokenizer
. Token IDs are copied to *GPU* memory
. The *GPU* runs the transformer forward pass (the expensive part)
. Results come back to the *CPU* for sampling and text conversion
. The *CPU* returns the response
...
. Training data had many sentences like "Eiffel Tower is in Paris"
. Those examples shaped the weights
. The weights now produce high probability for "Paris" given this context
...
|`/v1/completions`
|Text completion (given a prompt, generate text)

|`/v1/chat/completions`
|Chat format (messages array with roles)
...
|`temperature`
|Randomness (0 = greedy, >1 = more random)
...
xref:10-training.adoc[Next: Training — How Intelligence Emerges →]
