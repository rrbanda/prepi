...
:description: Understanding the KV cache optimization that makes autoregressive generation practical
...
*You might be wondering:* "_Why does the Query (Q) change for each new token, but K and V don't? Aren't they all computed the same way?_"
...
. *Dynamic allocation*: Allocate memory blocks only when needed, not upfront
. *Token tracking*: Know which tokens go where in memory
. *Smart reuse*: Reuse memory from finished requests for new ones
. *Eviction*: Remove stale data when memory fills up
. *Block mapping*: Maintain logical-to-physical address translation
...
|BlockSpaceManager Approach

|Pre-allocate max context length
...
|Wasted space for short requests
|Minimal waste

|Limited concurrent requests
|Many more concurrent requests
|===
...
*You might be wondering:* "_Why does GPT-3 need 4.7 MB per token while GPT-2 Small needs only 37 KB?_"
...
|*Grows with length*
|More tokens = bigger cache

|*Lives in GPU RAM*
|Fast access, limited space

|*Per-layer*
|Every transformer layer has its own cache
|===
...
|No, it's per-request and temporary

|"It stores knowledge"
|No, it stores computed K/V vectors

|"It persists between chats"
|No, it's created fresh each time

|"It's part of the model weights"
|No, weights are separate
|===
...
* *KV cache* stores the K/V vectors that _enable_ efficient context computation
* *Context* (contextual representation) is the _result_ of computing attention over those K/V vectors
...
_"Why does inference get slower as conversations get longer?"_

Three reasons:

. *Attention is O(n²)*: Each token attends to all previous tokens. More tokens = quadratically more attention computation.
. *KV cache grows*: More tokens = more memory for the cache. Eventually you hit memory limits.
. *Memory bandwidth*: Reading a larger KV cache takes more time, even if you have the memory.
...
xref:12-serving-systems.adoc[Next: Serving Systems — vLLM and Friends →]
